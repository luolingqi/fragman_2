...
$js = '
   ...
   var probes = {
      init: function(){
         if ( "" === $("#paramkey").attr("value") ) {
         $("input:radio[name=probeset]")[0].checked=true;
         $("#customize_probes").hide();
         } else {
         $("input:radio[name=probeset]")[1].checked=true;
         $("#customize_probes").show();
	 }
      }
   };

function cleartext(element){
	if (element.value == "#   SMILES"){
		element.value = "";
	}
};

function validate(){
	if ($("#orig").attr("checked")) {
			$("#loading").show();
			return true;
		} else {
		var x=$("#smiles_box").val().trim();
		if (x == "#   SMILES" || x == ""){
			return true;
		}
		x = x.replace(/\s+\n/, "\n");
		...
		var xs=x.split("\\n");
		if (xs.length>10){
			alert("Parameterization is limited to 10 probes at a time.");
			return false;
		}
		var patt = new RegExp("^[+-]?\\\\d+[\\\\ \\\\t]+\\\\S{2,}$");
		for (i in xs){
			if (!(patt.test(xs[i]))){
				alert("The given molecule \""+xs[i]+"\" is not of the correct format \"#     SMILES\"");
				return false;
				}
			}
		$("#loading").show();
...
$(document).ready(function(){
   ...
   $("#show_upload_tar").click(function() {
	   $("div#input_smiles").hide();
	   $("div#input_by_line").hide();
	   $("div#upload_tar").show();
	   });
...
<label style="float:none"><input type="radio" id="orig" name="probeset" value="orig" onClick='$("#customize_probes").hide()'>Original</label>
<label style="float:none"><input type="radio" id="customize" name="probeset" value="customize" onClick='$("#customize_probes").show()'>Upload Small Molecules</label>
</div>
...
<!--	<a href="#" id='opener2'>?</a><br/> -->
	...
	<br>......</p>
	<p>Example (Thrombin Inhibitor):<br/>
	0  NCc1cccc(Cl)c1
	</p>
	</div>
