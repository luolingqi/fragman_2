...
for my $line (@pdblines) {
    ...
    if ( $record eq "ATOM  " || $record eq "HETATM" ) {
        ...
        if ( $resname eq "HOH" || $resname eq "WAT" || $resname eq "ACE" || $resname eq "NME" ) {
            next;
        }
        if (   $resname eq "MSE"   #selenomethionine
            || $resname eq "PTR"   #phosphotyrosine
            || $resname eq "HRG"   #homoarginine
            || $resname eq "DPP"   #diaminopropanoic acid
            || $resname eq "HSD" ) #his protonation
        {
            $record = "ATOM  ";
        }
        if ( $resname eq "ASH" ) {
        ...
        if ( $resname eq "UNK" ) { #treat unknown residues as alanine
            $resname = "ALA";
            substr( $line, 17, 3 ) = "ALA";
        }
        ...
        $chain      = "h$hetresname".$chain if ( $record eq "HETATM" and not ($hetresname eq "hem" and $hem));
        if ( $ofile eq "NULL" ) {
            ofile();
