...
while(1)
{
   ...
   switch ($job->status) {
	...
	#check how many things are already running pbpdb.pl
          ...
          $cwd = getcwd();
          chdir($local_dir);
          $cmd = "apbspdbn.pl /home/ftmap/prms/beta/pdbamino.rtf /home/ftmap/prms/beta/parm.prm ./ protorig_nmin.pdb newtors ";
		...
		echo $cmd, "\n";
         system($cmd);
          ...
          if(! file_exists("protorig_nmin_apbs_pot_$job->pbmode.dx")) {
              $job->le('Unable to create PB Potential');
          }
...
#         system($cmd);
	 ...
	 chdir('probes');
		...
		chdir("$local_dir/probes/$probeid");
                ...
                $cmd = 'cat 1rec_for_min.pdb 1lig.ms > 1rec1lig.pdb';
                echo $cmd, "\n";
                system($cmd);
					...
					fclose($wc);
                                        ...
                                        if ($lines != $lines1)
                                        {
                                            ...
                                            if ($lines != 1 ) { # we haven't tried 128G
                                                ...
                                                $cmd .=	SCV_BIN . "/ace_min.mpi 1rec1lig_nmin.psf parm_new.prm pdbamino_new.rtf /dev/null 1rec1lig.pdb 1rec_for_min.pdb 0.5 $prms_dir/$job->rotsfile 1lig.pdb ftfinal ~tanggis/src/ace_minmpi/trunk/allh.tl 1lig.pdb";
                                                echo "cmd: $cmd\n";
	...
	if (file_exists('protorig_join.pdb')) {
      ...
      case 'c.f':
         ...
         else { // calculate contacts for nucleic acids
