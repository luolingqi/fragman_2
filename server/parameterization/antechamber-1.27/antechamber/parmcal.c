	...
	for (;;) {
		...
		if (aindex == 1) {
			...
			if(angleparmnum >= 10 * MAXATOMTYPE * MAXATOMTYPE) {
				fprintf(stderr, "Error, the number of angle parameters (%ld) exceed 10 * MAXATOMTYPE * MAXATOMTYPE, increase MAXATOMTYPE (%d) in define.h, angleparmnum, MAXATOMTYPE\n");
			}
		}
	}
	fclose(fp);
