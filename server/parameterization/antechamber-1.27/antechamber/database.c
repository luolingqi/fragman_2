 ...
 */
   ...
   random_flag +=2;
   if(random_flag >=random_array_size) random_flag = 0;
 /*  printf("\n time is %5d",now);  */
   randomr=&randoms;
...
int runcommand(char *commandstr)
{
	...
	if(status != 0) {
		...
		fprintf(fperror, "Error: cannot run \"%s\" in runcommand() of database.c properly, exit\n", realcommand);
		fflush(fperror);
		return 1;
	}
	return 0;
...
void rdef(char *filename)
{
	...
	if(debug) {
		...
		if(def_index ==1) {
			...
			printf("\nMolname defined starting Id = %d", id);
		}
		if(molname_field_index ==1) 
		...
		if(molname_fr >3) 
			printf("\nFor the first record, the %dth line is taken as the reference line for record name reading", molname_fr);
		printf("\nThe distance of the reference line to the line where to read record names: %d", molname_ld); 
		...
		for(i=0;i<add_line_num;i++)
			printf("\nAdd line (%d) =  %s (%d)", i+1, add_line[i], add_line_flag[i]);
	...
	if(separator_index == 1) {
		...
		if(process_flag == 1)
			...
			if ((fptmp = fopen(tmp_file, "w")) == NULL) {
				printf("\n Cannot open file %s ,exit", tmp_file);
				exit(0);
			}
			fileopen_flag = 1;
...
/* read in molname for the first record if molname_fr is applied*/
	...
	for (;;) {
		...
		if (fgets(line, MAXCHAR, fpin) == NULL) {
			if (process_flag == 1 && separator_index == 1 && separator_type == 0) {
/*add lines to the end of the file*/
				...
				printf("\nProcessing: %-8ld id=%7d, comp. name = %s\n", cmpd_num, id - 1, molname);
				fflush(stdout);
				fprintf(fpprocess, "\nProcessing: %-8ld id=%7d, comp. name = %s\n", cmpd_num, id - 1, molname);
				fflush(fpprocess);
/*run commands */
...
/*get record name if molname_field_index == 1 for most of cases*/
		if (molname_field_index== 1 && strstr(line, field) != 0) 
			if(molname_flag <= 1 || (molname_flag == 2 && molname_ld <=0) || (molname_flag == 3 && molname_ld <=0)) { 
			if (molname_ld == -5) 
...
/*get record name */
					...
					printf("\nProcessing: %-8ld id=%7d, comp. name = %s", cmpd_num, id - 1, molname);
					fflush(stdout);
					fprintf(fpprocess, "\nProcessing: %-8ld id=%7d, comp. name = %s", cmpd_num, id - 1, molname);
					fflush(fpprocess);
/*run commands */
...
/*get record name if molname_field_index == 1 for some cases*/
		if (molname_field_index== 1 && strstr(line, field) != 0) 
			if(molname_flag == 2 && molname_ld > 0 || molname_flag == 3 && molname_ld > 0) {
			strcpy(linebak5, linebak4);
			strcpy(linebak4, linebak3);
			strcpy(linebak3, linebak2);
			strcpy(linebak2, linebak1);
		...
		if(process_flag == 1)
			if(rend_line_flag == 1 || separator_line_flag == 1 && separator_type == 1)	
				continue;
			else
				fprintf(fptmp,"%s", line);
...
int main(int argc, char *argv[]) {
	...
	if ((fperror = fopen("error.log", "w")) == NULL) {
		printf("\n Cannot open fiel \"error.log\", exit\n");
		exit(0);
	}
	...
	if(proc_prob != -1) {
		...
		if (randomp == NULL) {
        		fprintf(stderr, "memory allocation error for *randomp\n");
        		exit(0);
		}
		random_id = 0;
		randomp[0] = 0;
