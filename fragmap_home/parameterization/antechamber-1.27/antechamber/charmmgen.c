	...
	for (i = 0; i < impropernum; i++) {
		...
		for (j = 0; j < improperparmnum; j++)
                    if (((improperparm[j].name1[0] == tmpc1[0]
                       ...
                       || improperparm[j].name1[0] == 'X'
                       && improperparm[j].name1[1] == ' ')
		...
		if (suc == 0) {
		                ...
		                tmpn1, tmpn2, tmpn3, tmpn4);
			fprintf(ffout, "%2s %2s %2s %2s%10.3lf%10.0lf%10.1lf",
				name1, name2, name3, name4, 1.1, 0.0, 180.0);
...
int main(int argc, char *argv[])
{
	...
	if (system_env != NULL) {
		...
		/*    printf("AMBERHOME environment not set, using gaff.dat in the current directory\n"); */
