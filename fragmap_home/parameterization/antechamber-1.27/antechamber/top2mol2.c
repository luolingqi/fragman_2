...
void ratcorr(void) {
	corratnum = 0;
		...
		if(corratnum > MAX_AT_CORR) {
			fprintf(stderr, "Error: the nubmer of corresponding atom type pairs (%d) exceed MAX_AT_CORR (%d), extends MAX_AT_CORR defined in top2mol2.c and recompile the program", corratnum, MAX_AT_CORR);
			exit(0);
		}
	}
}
void rbtcorr(void) {
	corrbtnum = 0;
		...
		if(corrbtnum > MAX_BT_CORR) {
			fprintf(stderr, "Error: the nubmer of corresponding bond type pairs (%d) exceed MAX_BT_CORR (%d), extends MAX_BT_CORR defined in top2mol2.c and recompile the program", corrbtnum, MAX_BT_CORR);
			exit(0);
		}
	}
}
...
for (i = 0; i < atomnum; i++) {
	...
	strcat(tmpc2, tmpc1);
	fprintf(fpout, "%7d %-8s%10.4lf%10.4lf%10.4lf %-6s%5d %-8s%10.4lf\n", tmpint, atomname[i].str, 
...
if(water_flag == 0)
	for (i = 0; i < resnum-resnum_water; i++) {
		...
		strcat(tmpc2, tmpc1);
		fprintf(fpout, "%6d %-7s %6d RESIDUE           1 A\n", i + 1, tmpc2, reshead[i]);
	}
if(water_flag == 1)
	for (i = 0; i < resnum; i++) {
		strcpy(tmpc2, "");
...
if (strcmp(COLORTEXT, "YES") == 0 || strcmp(COLORTEXT, "yes") == 0) {
	...
	if (strcmp(argv[1], "-h") == 0 || strcmp(argv[1], "-H") == 0) {
		printf("\n Usage top2mol2 -i  topology file name");
...
if (system_env != NULL) {
	strcpy(atfilename, system_env);
        strcat(atfilename, "/dat/antechamber/ATOMTYPE_CHECK.TAB");
	strcpy(btfilename, system_env);
        strcat(btfilename, "/dat/antechamber/BONDTYPE_CHECK.TAB");
} else {
        strcpy(atfilename, "ATOMTYPE_CHECK.TAB");
        strcpy(btfilename, "BONDTYPE_CHECK.TAB");
}
...
if((fpinp=fopen(pfilename,"r"))==NULL) {
    	fprintf(stderr, "\n Cannot open file %s, exit", pfilename);
	return 0;
}
if((fpinc=fopen(cfilename,"r"))==NULL) {
    	fprintf(stderr, "\n Cannot open file %s, exit", cfilename);
    	return 0;
}
...
if (bondat1_h == NULL) {
	fprintf(stderr, "memory allocation error for *bondat1_h\n");
	exit(0);
 }

bondat2_h = (long *) malloc(sizeof(long) * (top_bondnum_h + 10));
if (bondat2_h == NULL) {
	fprintf(stderr, "memory allocation error for *bondat2_h\n");
	exit(0);
 }

bondat1_noh = (long *) malloc(sizeof(long) * (top_bondnum_noh + 10));
if (bondat1_noh == NULL) {
        fprintf(stderr, "memory allocation error for *bondat1_noh\n");
        exit(0);
 }
                                                                                                                                                                            
bondat2_noh = (long *) malloc(sizeof(long) * (top_bondnum_noh + 10));
if (bondat2_noh == NULL) {
        fprintf(stderr, "memory allocation error for *bondat2_noh\n");
        exit(0);
 }

resno = (long *) malloc(sizeof(long) * (top_atomnum + 10));
if (resno == NULL) {
	fprintf(stderr, "memory allocation error for *resno\n");
	exit(0);
 }

reshead = (long *) malloc(sizeof(long) * (top_resnum + 10));
if (reshead == NULL) {
	fprintf(stderr, "memory allocation error for *reshead\n");
	exit(0);
 }

charge = (double *) malloc(sizeof(double) * (top_atomnum + 10));
if (charge == NULL) {
	fprintf(stderr, "memory allocation error for *charge\n");
	exit(0);
 }

coord_x = (double *) malloc(sizeof(double) * (top_atomnum + 10));
if (coord_x == NULL) {
	fprintf(stderr, "memory allocation error for *coord_x\n");
	exit(0);
 }

coord_y = (double *) malloc(sizeof(double) * (top_atomnum + 10));
if (coord_y == NULL) {
	fprintf(stderr, "memory allocation error for *coord_y\n");
	exit(0);
 }

coord_z = (double *) malloc(sizeof(double) * (top_atomnum + 10));
if (coord_z == NULL) {
	fprintf(stderr, "memory allocation error for *coord_z\n");
	exit(0);
 }
...
if (bondtype_h == NULL) {
	fprintf(stderr, "memory allocation error for *bondtype_h\n");
	exit(0);
 }

bondtype_noh = (STR_ARRAY *) malloc(sizeof(STR_ARRAY) * (top_bondnum_noh + 10));
if (bondtype_noh == NULL) {
	fprintf(stderr, "memory allocation error for *bondtype_noh\n");
	exit(0);
 }

atomname = (STR_ARRAY *) malloc(sizeof(STR_ARRAY) * (top_atomnum + 10));
if (atomname == NULL) {
	fprintf(stderr, "memory allocation error for *atomname\n");
	exit(0);
 }
...
if (atomtypebak == NULL) {
	fprintf(stderr, "memory allocation error for *atomtypebak\n");
	exit(0);
 }

atomres = (STR_ARRAY *) malloc(sizeof(STR_ARRAY) * (top_atomnum + 10));
if (atomres == NULL) {
	fprintf(stderr, "memory allocation error for *atomres\n");
	exit(0);
 }
