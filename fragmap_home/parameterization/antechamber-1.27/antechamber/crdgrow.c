...
/* for connect.tpl file */
	...
	if (system_env != NULL) {
		connect_file[0] = '\0';
		strcpy(connect_file, system_env);
		strcat(connect_file, "/dat/antechamber/CONNECT.TPL");
	} else {
		/*    printf("AMBEHROME environment not set, using CONNECT.TPL in the current directory\n"); */
		strcpy(connect_file, "CONNECT.TPL");
	}
...
/* memory allocation*/
	...
	if (index == NULL) {
		fprintf(stderr, "memory allocation error for *index\n");
		exit(0);
	}
